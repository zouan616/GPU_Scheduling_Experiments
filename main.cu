...
//task number
int n = 9; 
struct task GPU_task[9];


struct timeval global_tv[9];
double global_start_time[9];
double global_memory_start_time[9];
double global_memory_finish_time[9];
double global_kernel_start_time[9];
double global_kernel_finish_time[9];
double global_duration[9];

// Scheduler order

// ------------------- alg-I--------------
int sched_order[] = {9,4,5,6,7,8,3,2,1};

// ------------------- alg-I*--------------
//int sched_order[] = {9,1,4,5,6,2,7,8,3};

// ------------------- alg-II--------------
//int sched_order[] = {9,1,2,3,4,5,6,7,8};

// ------------------- alg-B--------------
//int sched_order[] = {9,4,5,6,7,8,1,3,2};
...
int main(int argc,char *argv[])
{
    ...
    //---------------for 7, 14, 21, 28 SMs--------------------------//
    kernel_length[i] = kernel_length[i] * 2;
    CTA_num[i] = CTA_num[i] * 2;
    }
    
   kernel_length[n-1] = 2000;
...
if(tt->task_num != n-1)
{
    ...
    //---------alg-II--------------------//
    /*
    if((tt->task_num != 0)&&(tt->task_num != 1)&&(tt->task_num != 8))
    {
